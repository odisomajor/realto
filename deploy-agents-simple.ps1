...
try {
    $result = ssh -i $sshKeyPath -o StrictHostKeyChecking=no root@146.190.121.74 $fullCommand
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Agents page deployed successfully!" -ForegroundColor Green
        Write-Host "🌐 Test at: https://xillix.co.ke/agents" -ForegroundColor Cyan
    } else {
